<template>
  <footer class="footer-section">
    <div class="container mx-auto px-4">
      
      <div class="footer-grid">
        
        <div class="footer-brand">
          <h3 class="brand-title">FINANZAPP</h3>
          <p class="brand-description">
            La herramienta más simple y efectiva para tomar el control de tus finanzas personales.
          </p>
          <p class="copyright">
            © {{ currentYear }} Todos los derechos reservados.
          </p>
        </div>

        <div class="footer-column">
          <h4 class="column-title">Navegación</h4>
          <ul class="footer-links">
            <li><router-link to="/" class="footer-link">Inicio</router-link></li>
            <li><router-link to="/acerca-de" class="footer-link">Acerca de</router-link></li>
            <li><router-link to="/servicios" class="footer-link">Servicios</router-link></li>
          </ul>
        </div>

        <div class="footer-column">
          <h4 class="column-title">Legal y Ayuda</h4>
          <ul class="footer-links">
            <li><router-link to="/contacto" class="footer-link">Contacto</router-link></li>
            <li><router-link to="/privacidad" class="footer-link">Política de Privacidad</router-link></li>
            <li><router-link to="/terminos" class="footer-link">Términos de Uso</router-link></li>
          </ul>
        </div>
        
        <div class="footer-column">
             <h4 class="column-title">Síguenos</h4>
             <div class="social-links">
                 <a href="#" aria-label="LinkedIn" class="social-link">
                   <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                     <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                   </svg>
                 </a>
                 <a href="#" aria-label="Twitter" class="social-link">
                   <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                     <path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/>
                   </svg>
                 </a>
                 <a href="#" aria-label="Instagram" class="social-link">
                   <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                     <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                   </svg>
                 </a>
             </div>
        </div>
      </div>

      <div class="footer-bottom">
          <p>Hecho con <Icon icon="pixelarticons:heart" width="24" height="24" class="heart-icon" /> para ayudarte a manejar tus finanzas.</p>
      </div>
      
    </div>
  </footer>
</template>

<script setup lang="ts">
import { ref } from 'vue';
import { Icon } from '@iconify/vue';
const currentYear = ref(new Date().getFullYear());



</script>

<style scoped>
.footer-section {
  background: linear-gradient(135deg, #2c3e50 0%, #35495e 100%);
  color: #e0e0e0;
  padding: 60px 0 20px;
  margin-top: 80px;
}

.footer-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));
  gap: 40px;
  margin-bottom: 40px;
}

.footer-brand {
  grid-column: span 1;
}

.brand-title {
  font-size: 1.75rem;
  font-weight: 900;
  color: #A2D3C7;
  margin-bottom: 16px;
  font-family: 'Arial Black', sans-serif;
  letter-spacing: 1px;
}

.brand-description {
  font-size: 0.95rem;
  line-height: 1.6;
  color: #b0b0b0;
  margin-bottom: 16px;
}

.copyright {
  font-size: 0.85rem;
  color: #888;
  margin-top: 12px;
}

.footer-column {
  display: flex;
  flex-direction: column;
}

.column-title {
  font-size: 1.15rem;
  font-weight: 700;
  color: #ffffff;
  margin-bottom: 16px;
}

.footer-links {
  list-style: none;
  padding: 0;
  margin: 0;
  display: flex;
  flex-direction: column;
  gap: 12px;
}

.footer-link {
  color: #b0b0b0;
  text-decoration: none;
  font-size: 0.95rem;
  transition: all 0.3s ease;
  display: inline-block;
}

.footer-link:hover {
  color: #A2D3C7;
  transform: translateX(5px);
}

.social-links {
  display: flex;
  gap: 16px;
}

.social-link {
  width: 40px;
  height: 40px;
  display: flex;
  align-items: center;
  justify-content: center;
  border-radius: 50%;
  background: rgba(162, 211, 199, 0.1);
  color: #A2D3C7;
  transition: all 0.3s ease;
}

.social-link:hover {
  background: #A2D3C7;
  color: #2c3e50;
  transform: translateY(-3px);
  box-shadow: 0 5px 15px rgba(162, 211, 199, 0.3);
}

.social-link svg {
  width: 20px;
  height: 20px;
}

.footer-bottom {
  padding-top: 30px;
  border-top: 1px solid rgba(162, 211, 199, 0.2);
  text-align: center;
}

.footer-bottom p {
  font-size: 0.9rem;
  color: #888;
}

@media (max-width: 768px) {
  .footer-grid {
    grid-template-columns: 1fr;
    gap: 30px;
  }
  
  .footer-section {
    padding: 40px 0 20px;
  }
  
  .brand-title {
    font-size: 1.5rem;
  }
}
.heart-icon {
  color: var(--color-extra);
  display: inline-block;
  vertical-align: middle;
  animation: heartbeat 1.5s ease-in-out infinite;;
}

</style>